...
class WorkerController(
    private val workerJobs: WorkerJobService,
    private val progressService: WorkerProgressService
) {
    @GetMapping("/health")
    fun health() = mapOf(
        "status" to if (workerJobs.isHealthy()) "healthy" else "unhealthy",
        "workerRunning" to workerJobs.isHealthy()
    )

    // The Python worker is intentionally managed outside Spring.
    // These endpoints remain harmless compatibility endpoints.
    @PostMapping("/start")
    fun start() = workerJobs.availability().fold(
    ...
    fun command(@RequestBody request: WorkerCommandRequest): ResponseEntity<Any> =
        when (request.command) {
            ...
            "cancel" -> request.jobId?.let(workerJobs::cancel)?.let {
                ResponseEntity.ok(WorkerCommandResponse(true, "Job cancelled", it.id))
            } ?: ResponseEntity.status(HttpStatus.NOT_FOUND).body(WorkerCommandResponse(false, "Job not found", request.jobId))
    ...
    fun getJob(@PathVariable jobId: String): ResponseEntity<Any> =
        workerJobs.get(jobId)?.let { ResponseEntity.ok<Any>(JobDTO.fromJob(it)) }
    ...
    fun recentJobs(@RequestParam(defaultValue = "10") limit: Int) =
        workerJobs.recent(limit).map { JobDTO.fromJob(it) }

    private fun submit(command: WorkerCommand): ResponseEntity<Any> {
        val job = workerJobs.submit(command)
