...
import app.melotrail.model.LoudnessReport
import app.melotrail.worker.AnalyzeCommand
import app.melotrail.worker.AnalyzeOptions
import app.melotrail.worker.HttpWorkerClient
import app.melotrail.worker.MP3ConvertCommand
...
class AudioPipeline(
    ...
    suspend fun run(): PipelineResult {
        ...
        val workDir = Files.createTempDirectory("melotrail-cli-")
    ...
    private fun decodeAudio(
        path: Path
    ): app.melotrail.audio.AudioBuffer? {
        return when {
            path.toString().endsWith(".wav", ignoreCase = true) -> {
                app.melotrail.audio.WAVDecoder(cliErrorReporter)
                    .decode(path)
            }

            path.toString().endsWith(".flac", ignoreCase = true) -> {
                app.melotrail.audio.FLACDecoder(cliErrorReporter)
                    .decode(path)
            }
