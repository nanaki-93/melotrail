...
/** One explicitly selected source track/channel is the protected melody authority. */
...
class MidiImportValidator {
    fun validate(inspection: MidiInspectionResult, context: MidiValidationContext = MidiValidationContext()): MidiImportValidationResult {
        val findings = buildList {
            ...
            validateMelody(inspection, context, this)
            validateReaderIssues(inspection.findings, context.selectedMelody, this)
            validateAdvisories(inspection.sequence, context, this)
    ...
    private fun validateMelody(inspection: MidiInspectionResult, context: MidiValidationContext, findings: MutableList<MidiFinding>) {
        val selection = context.selectedMelody ?: run {
            findings += finding(MidiFindingCode.MELODY_SELECTION_REQUIRED, MidiFindingSeverity.AWAITING_AUTHORITY, MidiFindingScope.MELODY_SELECTION,
                "Select one source track and channel as the protected melody.", "Choose the melody track and channel before arranging.")
            return
        }
        ...
        if (channel == null) {
            findings += finding(MidiFindingCode.MELODY_SELECTION_NOT_FOUND, MidiFindingSeverity.BLOCKING, MidiFindingScope.MELODY_SELECTION,
                "The selected melody track or channel is not present in the imported MIDI.", "Select a track/channel reported by MIDI inspection.", selection.trackIndex, selection.channel)
            ...
            findings += finding(MidiFindingCode.MELODY_HAS_NO_NOTES, MidiFindingSeverity.BLOCKING, MidiFindingScope.MELODY_SELECTION,
                "The selected melody channel contains no complete notes.", "Select a channel with melody notes.", selection.trackIndex, selection.channel)
        }
    }
    ...
    private fun validateReaderIssues(issues: List<MidiReaderIssue>, selection: MidiMelodySelection?, findings: MutableList<MidiFinding>) {
        issues.forEach { issue -> when (issue.code) {
            ...
            MidiReaderIssueCode.UNCLOSED_NOTE_ON -> findings += finding(MidiFindingCode.UNSAFE_SELECTED_MELODY_PAIRING,
                if (selection?.trackIndex == issue.trackIndex) MidiFindingSeverity.BLOCKING else MidiFindingSeverity.ADVISORY,
                MidiFindingScope.EVENT, issue.message,
                if (selection?.trackIndex == issue.trackIndex) "Select a safely paired melody track or repair its missing note-off." else "Review this reference-track note pairing if you need it for audition.",
    ...
    private fun validateAdvisories(sequence: SemanticMidiSequence, context: MidiValidationContext, findings: MutableList<MidiFinding>) {
        ...
        if (notes.any { left -> notes.any { right -> left !== right && left.orderingKey.tick < right.endTick && right.orderingKey.tick < left.endTick } }) {
            findings += finding(MidiFindingCode.POLYPHONY, MidiFindingSeverity.ADVISORY, MidiFindingScope.CHANNEL,
                "The selected melody channel is polyphonic.", "Review the melody selection; polyphony is preserved and is not rejected.", selection.trackIndex, selection.channel)
        }
