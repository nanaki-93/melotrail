...
class DeterministicStemMixer {
    fun mix(tracks: List<MixTrack>, settings: MixSettings = MixSettings()): MixedStem {
        ...
        require(outputChannels in 1..2) { "The simple stem mixer supports mono and stereo tracks only" }
    ...
    private fun validateTrack(track: MixTrack) {
        ...
        require(track.buffer.format.channels in 1..2) {
            "Mix track '${track.name}' must be mono or stereo"
        }
    ...
    private fun convertChannels(buffer: AudioBuffer, targetChannels: Int): AudioBuffer {
        ...
        for (frame in 0 until buffer.length) {
            if (targetChannels == 2) {
                val sample = buffer.getSample(0, frame)
                output[frame * 2] = sample
                output[frame * 2 + 1] = sample
            } else {
                output[frame] = (buffer.getSample(0, frame) + buffer.getSample(1, frame)) / 2f
            }
        }
        return AudioBuffer(
            samples = output,
