...
import ai.music.workstation.arrangement.MixedStem
...
import ai.music.workstation.preparation.AudioCleanupBoundary
import ai.music.workstation.preparation.AudioCleanupRequest
import ai.music.workstation.preparation.AudioCleanupResult
import ai.music.workstation.preparation.AudioInspectionMeasurements
import ai.music.workstation.preparation.CleanupMetrics
import ai.music.workstation.preparation.DetectedAudioFormat
import ai.music.workstation.preparation.DetectedInput
import ai.music.workstation.preparation.EvidenceLevel
...
import ai.music.workstation.preparation.SignalIndicator
import ai.music.workstation.preparation.SilenceEvidence
import ai.music.workstation.preparation.TranscriptionBoundary
import ai.music.workstation.preparation.TranscriptionBoundaryResult
import ai.music.workstation.preparation.TranscriptionEngineMetadata
...
class EndToEndWorkflowCompatibilityTest {
    ...
    fun `direct MIDI clean WAV noisy WAV and MP3 reach validated releases without source mutation`() = runBlocking {
        ...
        cases.forEach { fixture ->
            ...
            if (fixture.noisy) {
                ...
                services.projects.retryMidiCleanup(RetryMidiCleanupRequest(root, "A", ai.music.workstation.arrangement.MidiCleanupOptions()))
    ...
    fun `legacy v1 and pre-provenance v2 projects remain readable beside approved v3 arrangements`() = runBlocking {
        ...
        ProjectStore.write(v2, Project(version = 2, name = "v2", parts = listOf(Part("A", "source/A.mid", midi = ai.music.workstation.arrangement.MidiReferences(clean = "midi/clean/A.mid"))), structure = listOf("A"), renderFormat = RenderFormat()))
    ...
    private fun report(request: ai.music.workstation.preparation.InputInspectionRequest): InputInspectionReport {
    ...
    private class FakeRenderer : InstrumentRenderer {
        override suspend fun render(midi: Path, instrument: LogicalInstrument, output: Path, format: RenderFormat, expectedFrames: Long): RenderResult {
            val audio = ai.music.workstation.audio.AudioBuffer(FloatArray((expectedFrames * format.channels).toInt()) { 0.2f }, ai.music.workstation.audio.AudioFormat(format.sampleRate, format.channels, 24, false, false, "WAV"), expectedFrames.toDouble() / format.sampleRate)
