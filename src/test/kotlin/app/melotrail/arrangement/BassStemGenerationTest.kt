...
class BassStemGenerationTest {
    ...
    fun `adapter writes and reparses one full timeline MIDI artifact`() {
        ...
        writeTestMidi(clean)
        val project = Project(Project.CURRENT_VERSION, "bass", listOf(Part("A", "source/A.mid", midi = canonicalMidiReferences(projectRoot, "A"))), renderFormat = RenderFormat())
    ...
    fun `adapter consumes detailed arrangement bass controls`() {
        ...
        Files.writeString(source, "source MIDI remains untouched")
        writeTestMidi(clean)
        val project = Project(Project.CURRENT_VERSION, "bass-v3", listOf(Part("A", "source/A.mid", midi = canonicalMidiReferences(projectRoot, "A"))), renderFormat = RenderFormat())
