...
suspend fun generateApprovedCohesion(root: Path) {
    val service = DefaultCohesionApplicationService { input ->
        TransitionCohesionPlan(inputHash = input.inputHash, model = CohesionModelIdentity("qwen", "test", "1".repeat(64)), boundaries = input.boundaries.map { boundary ->
            TransitionBridgePlan(boundary.outgoingInstanceId, boundary.incomingInstanceId, boundary.outgoing.sourceHash, boundary.incoming.sourceHash,
    ...
    draft.boundaries.forEach { service.reviewBoundary(root, it.outgoingInstanceId, it.incomingInstanceId) }
