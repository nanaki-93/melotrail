...
flowchart LR
    ...
    C --> H[Author Verse / Chorus / Bridge harmony]
    H --> G[Generate and approve arrangement]
...
| 13. **Release export and commercial evidence** — use **Export Song**; when commercial use is intended, use **Create commercial evidence** | **Export Song** needs a validated `output/master.wav` and `output/release.json`. WAV is always available; MP3 additionally requires the local worker encoder. Its destination is the project `output/` folder and its filename cannot overwrite a protected artifact. Commercial evidence additionally needs an honest ownership, commercial-permission, or public-domain attestation for every source plus reviewed permitted terms for used models, libraries, and samples. | A separately named final WAV copy or MP3 in `output/`; `master.wav` remains the canonical lossless release. **Create commercial evidence** writes `provenance-manifest.json`, `commercial-report.md`, and `youtube-upload-checklist.md`. | No. Export verifies that the authoritative master did not change. Commercial evidence records hashes and attestations; it does not change sources. | A stale live lineage disables new build output, but a historical `release.json` whose master fingerprint and measured metadata still validate remains exportable without rebuilding. A missing or mismatched manifest requires a new build. The workflow's final commercial-review stage remains blocked until every source has an attestation; correct the source-rights record, regenerate commercial evidence, and review its warnings. It is evidence, not legal clearance. |
