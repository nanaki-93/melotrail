...
| 9. **Arrangement** — use **Generate arrangement**, then approve a draft when shown | Current analyses, saved structure, current Source Song Critic approval, and allowed logical instruments (including piano). | `song_plan.json`, `section_variations.json`, and either approved `arrangement_plan.json` or review-only `arrangement_plan.draft.json`. Approval records exact structure, occurrence, context, and plan hashes. | No source or MIDI changes. | An arrangement rerun invalidates cohesion and every later build artifact. A Qwen draft is not usable until you explicitly approve it; it is advice, not an executable action. |
| 9. **Ensemble Cohesion** — use **Generate Ensemble Cohesion** after Arrangement approval, review every boundary, then **Approve Ensemble Cohesion** | A current approved arrangement plus its exact saved structure and MIDI analyses. | Reviewable, path-free `TransitionContext` plans under `cohesion/`, with one deterministic pattern-library bridge MIDI/audit record per adjacent occurrence pair plus aggregate draft/approval records. A one-occurrence structure has an explicit empty result. | No source or selected MIDI changes. Any allowed melody edit is a separate occurrence artifact, restricted to its boundary bar and protected anchors. | A changed arrangement, selected MIDI, analysis, or structure makes Ensemble Cohesion stale; historical target-order cohesion remains inspectable only. |
...
| 13. **Release export and commercial evidence** — use **Export Song**; when commercial use is intended, use **Create commercial evidence** | **Export Song** needs a validated `output/master.wav` and `output/release.json`. WAV is always available; MP3 additionally requires the local worker encoder. Its destination is the project `output/` folder and its filename cannot overwrite a protected artifact. Commercial evidence additionally needs an honest ownership, commercial-permission, or public-domain attestation for every source plus reviewed permitted terms for the exact used models and stems. Select and confirm a signature-motif phrase, then run its deterministic recognizability gate; it requires one clear surviving piano occurrence. | A separately named final WAV copy or MP3 in `output`; `master.wav` remains the canonical lossless release. The motif report is `motif/<source-sha256>/<input-sha256>/report.json`. **Create commercial evidence** writes immutable `output/releases/<releaseId>/provenance.json`, `commercial-report.md`, and `youtube-release.json`. | No. Export verifies that the authoritative master did not change. Commercial evidence records a selected hash-bound lineage; it does not change sources. | A missing, stale, or failed signature-motif gate, or any other unresolved dependency, blocks the commercial-ready claim but never locks or deletes project evidence. Historical rejected, failed, and stale stage records remain inspectable outside the selected lineage. It is evidence, not legal clearance. |
