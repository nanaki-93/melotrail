# Agent Prompt — Milestone 11: Core Approval + Optional Layers
...
- Read PLAN.md.
- Read the V2 roadmap milestone file for Milestone 11: Core Approval + Optional Layers.
...
Required outcome:
- Add persisted core-arrangement approval.
- Introduce section `DensityBudget`.
- Generate strings only when remaining density permits.
- Prefer sustained harmony/climax support before countermelody.
- For countermelody, use melodic-gap / call-response rules.
- Validate melody collisions and register occupation.

Success criteria:
- Optional layers cannot run before core approval.
- Strings may explicitly resolve to OFF.
- Countermelody does not continuously compete with source melody.
- Density budget is visible in critic/debug output.
