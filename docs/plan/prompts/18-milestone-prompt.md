# Agent Prompt — Milestone 18: Release Similarity / Originality Critic
...
- Read PLAN.md.
- Read the V2 roadmap milestone file for Milestone 18: Release Similarity / Originality Critic.
...
Required outcome:
- Create a release fingerprint for every completed song.
- Compare structure fingerprint.
- Compare energy curve.
- Compare instrument-entry/exit sequence.
- Compare bass-pattern sequence.
- Compare drum-groove sequence.
- Compare transition sequence.
- Compare tempo/swing profile.
- Compare arrangement-density curve.
- Produce similarity score plus specific explanations.
- Store the report in release metadata.
- High similarity produces a warning, not an automatic YouTube-policy verdict.
- Optional Qwen re-plan may change arrangement/groove/orchestration while preserving human melody, harmony and structure.
- Never randomize parameters solely to "beat" a policy detector.

Success criteria:
- Release fingerprint is deterministic.
- Similarity score is accompanied by interpretable feature-level explanations.
- Human melody/harmony/structure are not silently changed.
- Re-plan targets arrangement strategy rather than authorship elements.
- Similarity review status is stored in the release manifest.
- The system never claims to determine YPP eligibility.
