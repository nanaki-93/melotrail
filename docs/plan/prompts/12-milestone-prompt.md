# Agent Prompt — Milestone 12: Transitions & Ensemble Cohesion
...
- Read PLAN.md.
- Read the V2 roadmap milestone file for Milestone 12: Transitions & Ensemble Cohesion.
...
Required outcome:
- Build `TransitionContext` for every section boundary.
- Use deterministic transition strategies from the pattern library.
- Allow Qwen to choose transition intent when useful.
- Rename/split current generic cohesion into explicit `EnsembleCohesion`.
- Restrict default edit windows to boundary bars.
- Protect source melody except for explicitly allowed transition edits.

Success criteria:
- Pre-arrangement Melody Connection remains separate.
- Ensemble Cohesion runs only after arrangement.
- Transition decisions are persisted.
- Melody identity is preserved.
- Regression test prevents the old cohesion-before-arrangement bug.
