# Agent Prompt — Milestone 15: Production Instrument Bank & Renderer
...
- Read PLAN.md.
- Read the V2 roadmap milestone file for Milestone 15: Production Instrument Bank & Renderer.
...
Required outcome:
- Add `productionApproved`, `qualityTier`, `styleAffinity`, `preferredRoles` to instrument metadata.
- Curate a small auditioned production bank.
- Keep renderer behind `InstrumentRenderer` abstraction.
- Support current SF2/SFZ path while allowing future VST3/Audio Unit implementations.
- Preserve license/provenance per used instrument.

Success criteria:
- Automatic production uses only approved instruments by default.
- Arrangement code does not depend on renderer implementation.
- Every rendered stem records instrument/license provenance.
