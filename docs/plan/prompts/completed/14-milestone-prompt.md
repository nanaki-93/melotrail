# Agent Prompt — Milestone 14: Signature Motif & Recognizability Gate
...
- Read PLAN.md.
- Read the V2 roadmap milestone file for Milestone 14: Signature Motif & Recognizability Gate.
...
Required outcome:
- Introduce `SignatureMotif` with user-confirmable phrase selection.
- Track lineage of source note IDs through transformations.
- Calculate interval contour similarity, rhythm similarity, anchor retention and matched-note coverage.
- Require at least one clear surviving occurrence of the main motif.
- Create debug report and release-gate result.

Success criteria:
- Signature motif can be selected or confirmed by the user.
- Recognizability score is reproducible.
- Anchor loss is visible.
- Commercial-ready build fails when configured thresholds are not met.
