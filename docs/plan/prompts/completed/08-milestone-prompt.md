# Agent Prompt — Milestone 08: Incremental Arrangement State
...
- Read PLAN.md.
- Read the V2 roadmap milestone file for Milestone 08: Incremental Arrangement State.
...
Required outcome:
- Introduce immutable/versioned `ArrangementState`.
- Start from accepted source/piano track.
- Add tracks only after validation passes.
- Provide onset/activity/register/density summaries for accepted tracks.
- Allow deterministic generators access to full accepted MIDI.
- Allow Qwen planners to receive compact summaries plus relevant excerpts.

Success criteria:
- Bass is generated from piano + musical context.
- Drums can access accepted bass attacks and piano activity.
- Pad can access ensemble register/density state.
- Strings can access complete accepted core arrangement.
- Rejected candidates never enter state.
