...
| Canonical full melody | `SourceSongAssembler` publishes an immutable v2 conductor plus one full-melody track, section markers, occurrence/harmony/window/lineage/hash sidecar evidence, and one `FullSongGrooveMap`; Melody Connection and Source Song Critic use its global identity. [`OccurrenceMidiArtifactResolver`](../../src/main/kotlin/app/melotrail/arrangement/OccurrenceMidiArtifactResolver.kt) still falls back to per-part selected MIDI. | The exact melody that was connected and criticized is not yet guaranteed to be the piano melody later arranged, humanized, rendered, and exported. | QP-007 complete; QP-008 |
