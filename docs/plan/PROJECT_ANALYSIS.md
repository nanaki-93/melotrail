...
| Melody monophony | [`MidiMonophonicMelodyPreparer`](../../src/main/kotlin/app/melotrail/arrangement/MidiMonophonicMelodyPreparation.kt) materializes per-channel sustain/all-notes state, reports every source note/controller decision, and publishes one note-bearing source candidate before [`SourceSongAssembler`](../../src/main/kotlin/app/melotrail/arrangement/SourceSong.kt) copies it into occurrence-local tracks. | Source sections are now globally monophonic, but the assembled song still has occurrence-local tracks rather than one canonical full melody consumed by every downstream stage. | QP-005 complete; QP-007–QP-008 |
