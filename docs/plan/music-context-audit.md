...
| --- | --- | --- | --- | --- | --- |
| Project identity, parts, source references, render format | `Project`, `SongPart`, `RenderFormat` | `project.json`, `@Serializable`; `ProjectStore` | Every application service and desktop snapshot | Canonical project envelope; `ProjectValidator` checks version, project-relative paths, and PCM-24 render constraints | Legacy v1–v3 reads remain supported. `src/main/kotlin/app/melotrail/arrangement/Project.kt`, `ProjectStore.kt` |
...
| Audio inspection facts | `PartAnalysis` | `analysis/<part>.json`; worker/import analysis via `PartAnalysisStore` | Import/readiness and desktop project snapshot | Descriptive only: duration, format, levels, inferred BPM/key and silence/onsets; no declared setting is overwritten | Separate audio analysis schema intentionally differs from MIDI analysis. `arrangement/PartAnalysis.kt`, `application/ProjectApplicationService.kt` |
...
| Arrangement plan, detailed arrangement, instrument assignments | `SongPlanningInput`, `SongPlan`, `DetailedArrangement`, `ArrangementAssignmentReference` | `song-plan.json`, `arrangement.json`, drafts/reviews; arrangement service/stores | Generators, Cohesion, render | Approved arrangement is hash-bound to structure, occurrence, planning context, and plan | Each service builds its own context; current arrangement critic runs before generated MIDI. Tasks 124, 125, and 127. `arrangement/GlobalSongPlanner.kt`, `arrangement/DetailedArrangement.kt`, `application/ArrangementApplicationService.kt`, `arrangement/ArrangementCritic.kt` |
...
| End-to-end evidence and compatibility rollout | Existing compatibility tests/readers cover current behavior; no deterministic reference-song fixture proves the target pipeline. | Task 130, after the preceding contracts exist. Evidence: `src/test/kotlin/app/melotrail/application/EndToEndWorkflowCompatibilityTest.kt`, `docs/COMPATIBILITY_READERS.md` |
