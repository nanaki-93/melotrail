...
class WorkspaceViewModelTest {
    ...
    fun `structure mutations preserve canonical identities reject ineligible parts and recover from save failure`() = runTest {
        ...
        viewModel.accept(WorkspaceIntent.ClearStructure)
        assertEquals(WorkspaceDialog.ConfirmClearStructure, viewModel.state.value.dialog)
        assertEquals(listOf("A", "A"), viewModel.state.value.structureDraft, "clear must not mutate before confirmation")
        viewModel.accept(WorkspaceIntent.ConfirmClearStructure); advanceUntilIdle()
    ...
    fun `canonical occurrence actions select and mutate by instance identity`() = runTest {
        ...
        viewModel.accept(WorkspaceIntent.RemoveStructureOccurrence("B2"))
        assertEquals(WorkspaceDialog.ConfirmPartStructureChange("B", "B2"), viewModel.state.value.dialog)
        assertEquals(listOf("B", "B", "A"), checkNotNull(service.savedStructure).partIds, "removal must not mutate before confirmation")
        viewModel.accept(WorkspaceIntent.ConfirmPartStructureChange); advanceUntilIdle()
        assertEquals(listOf("B", "A"), checkNotNull(service.savedStructure).partIds)
        assertEquals(WorkspaceSection.STRUCTURE, viewModel.state.value.workspaceSection)
        viewModel.close()
    }
